...
// This is quite ugly workaround for the rest of the code, that expects DV RPU
// and metadata on the base track. In dual track files, those are attached to EL
// track generally, so we copy over the metadata to base track. Refactoring this
// to look also at EL for metadata everywhere is bit annoying, so for now, let's
